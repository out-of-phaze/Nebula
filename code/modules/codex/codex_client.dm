...
/client/verb/search_codex(searching as text)
	...
	if(LAZYLEN(all_entries) == 1)
		...
		if(LAZYLEN(all_entries) > 1)
			...
			codex_data += "</table>"
			var/datum/browser/popup = new(mob, "codex-search", "Codex Search", ndel_on_close = TRUE)
...
/client/verb/list_codex_entries()
	...
	var/datum/browser/popup = new(mob, "codex-index", "Codex Index", ndel_on_close = TRUE)

	...
	switch(val)
		if(2 to 3)
			if(prob(60))
				...
				new grass_path(T)
				T.contents_were_modified("desert grass")
			if(prob(5))
				...
				new mob_type(T)
				T.contents_were_modified("desert vermin")
		if(5 to 6)
			if(prob(20))
				var/grass_path = pick(subtypesof(/obj/structure/flora/grass))
				new grass_path(T)
				T.contents_were_modified("desert grass rare")
		if(7 to 9)
			if(prob(60))
				new /obj/structure/flora/bush(T)
				T.contents_were_modified("desert bush")
			else if(prob(20))
				new /obj/structure/flora/tree/dead(T)
				T.contents_were_modified("desert dead tree")

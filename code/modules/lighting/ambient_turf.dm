...
/turf/proc/replace_ambient_light(old_color, new_color, old_multiplier, new_multiplier = 0)
	...
	var/list/old_parts = _ambient_rgb2num_turf_cached(old_color)
	var/list/new_parts = _ambient_rgb2num_turf_cached(new_color)

	var/dr = new_parts[1] * new_multiplier - old_parts[1] * old_multiplier
	var/dg = new_parts[2] * new_multiplier - old_parts[2] * old_multiplier
	var/db = new_parts[3] * new_multiplier - old_parts[3] * old_multiplier
...
/turf/proc/add_ambient_light(color, multiplier, update = TRUE)
	...
	var/list/ambient_parts = _ambient_rgb2num_turf_cached(color)

	var/ambient_r = ambient_parts[1] * multiplier
	var/ambient_g = ambient_parts[2] * multiplier
	var/ambient_b = ambient_parts[3] * multiplier
...
/turf/proc/update_ambient_light(no_corner_update = FALSE)
	...
	if (ambient_light)
		var/list/parts = _ambient_rgb2num_turf_cached(ambient_light)
		ambient_r = (parts[1] * ambient_light_multiplier) - ambient_light_old_r
		ambient_g = (parts[2] * ambient_light_multiplier) - ambient_light_old_g
		ambient_b = (parts[3] * ambient_light_multiplier) - ambient_light_old_b
	else
